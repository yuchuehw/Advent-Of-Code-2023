...
Task: Figure out the shortest distance of each pair of stars!
"""
...
def find_min_distance_of_all_pairs(file_path: str) -> int:
    with open(file_path, "r", encoding="utf8") as file:
        ...
        for star_i, star_j in combinations(stars, 2):
            ...
            distance += sum(1 if i not in empty_rows else 2 for i in range(a + 1, b + 1, 1))

            a, b = min(star_i[1], star_j[1]), max(star_i[1], star_j[1])
            distance += sum(1 if j not in empty_cols else 2 for j in range(a + 1, b + 1, 1))
